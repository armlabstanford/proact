...
folder_path = os.path.expanduser('~/ws_arm/src/arm-sim/box_blocks_experiment/config/')
...
for i in range(8): # participant
    for j in range(3): # trial
        locations = plan[j][i]        
        data = {}
        for name, value in zip(variable_names, locations):
            data[name] = value
        file_path = os.path.join(folder_path, f'participant{i+1}_trial{j+1}.yaml')
